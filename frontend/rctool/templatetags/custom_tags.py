...
def fit_data(
    ...
    *args
    ...
    field_data = [
        [a, b]
        for a, b in zip(df_field["stage"].tolist(), df_field["discharge"].tolist())
    ]
    rc_data_output = [
        ...
        if breakpoint1 and breakpoint1 > x_min and breakpoint1 < x_max:
            ...
            if (
                ...
                [mdl_data_upper, mdl_param_upper] = fit_linear_model(
                    df_upper, offsets[1], "model segment 2", intersect_point_1
                )
        ...
        # if no breakpoint specified, iterate between numerous breakpoints and optimize fit
            ...
            for k in np.arange(x_min, x_max, x_step):
                ...
                if len(df_lower["discharge"]) > 1 and len(df_upper["discharge"]) > 1:
                    ...
                    rmse = np.mean(
                        [mdl_param_lower["wgt"]["rmse"], mdl_param_upper["wgt"]["rmse"]]
                    )
